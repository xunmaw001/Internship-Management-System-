        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '实习作业标题' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.shixizuoyeName" placeholder="实习作业标题" clearable></el-input>
                     </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('shixizuoye','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/shixisheng/upload/shixizuoyeMuBan.xls"
                        >批量导入实习作业数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('shixizuoye','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="shixizuoyeUploadSuccess"
                                :on-error="shixizuoyeUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('shixizuoye','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入实习作业数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('shixizuoye','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "shixizuoye.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="shixizuoyeUuidNumber"
                                   header-align="center"
                                   label="实习作业编号">
                        <template slot-scope="scope">
                            {{scope.row.shixizuoyeUuidNumber}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   ...
                                   label="实习作业标题">
                        <template slot-scope="scope">
                            {{scope.row.shixizuoyeName}}
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="shixizuoyeFile"
                                     header-align="center"
                                     width="200"
                                     label="作业文件">
                        <template slot-scope="scope">
                            <div v-if="scope.row.shixizuoyeFile">
                                <a type="text" style="text-decoration:none" class="el-button" :href="$base.url+scope.row.shixizuoyeFile">下载</a>
                            </div>
                            <div v-else>无文件</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="shixizuoyeOldMoney"
                                   header-align="center"
                                   label="作业打分">
                        <template slot-scope="scope">
                            {{scope.row.shixizuoyeOldMoney}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('shixizuoye','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('shixizuoye','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="sessionTable=='fudaoyuan' && scope.row.shixizuoyeOldMoney==null" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">评分</el-button>
                            <el-button v-if="isAuth('shixizuoye','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            form:{
                id : null,
                yonghuId : null,
                shixizuoyeUuidNumber : null,
                shixizuoyeName : null,
                shixizuoyeContent : null,
                shixizuoyeFile : null,
                insertTime : null,
                shixizuoyeOldMoney : null,
                shixizuoyePingyuContent : null,
                updateTime : null,
                createTime : null,
            },
            dataList: [],
            ...
            json_fields: {
                //级联表字段
                     ...
                     '邮箱': 'yonghuEmail',
                //本表字段
                     '实习作业编号': "shixizuoyeUuidNumber",
                     '实习作业标题': "shixizuoyeName",
                     '作业文件': "shixizuoyeFile",
                     '上传时间': "insertTime",
                     '作业打分': "shixizuoyeOldMoney",
                     '评论时间': "updateTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'shixizuoye',//当前表表名,
                        sumColum : 'shixizuoye_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'shixizuoye_name',//分组字符串字段
                        // types : 'shixizuoye_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.shixizuoyeName!= '' && this.searchForm.shixizuoyeName!= undefined) {
                    params['shixizuoyeName'] = '%' + this.searchForm.shixizuoyeName + '%'
                }
                                                        
                params['shixizuoyeDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            shixizuoyeUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "shixizuoye/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            wuyong(id) {
                ...
                _this.$confirm(`确定    操作?`, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        url: "shixizuoye/update",
                        method: "post",
                        data: {
                            id:id,
//                            shixizuoyeTypes:shixizuoyeTypes,
                        }
                    }).then(({data}) => {
                        if(data && data.code === 0){
